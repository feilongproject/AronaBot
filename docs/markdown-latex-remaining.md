...
| `chatbot.ts` | 发送层「语言优化」：仅对 4 条固定系统短句套 `\textcolor`（行内 `$...$` 左对齐）——限流（`#F39C12`）、看不了图（`#7F8C8D`）、门控拒绝（`#E74C3C`）、闭嘴确认（`#5DADE2`）。门控台词池 / 闭嘴文案为运营自定义，含 `$ \ {}` 或超 60 字不进公式原样发；`allowMarkdown` 关闭时发纯文本。模型回复主体不进公式：固定输出协议明确禁止模型输出 MD / LaTeX；历史装配 / 引用块 / 压缩 transcript 对 assistant 行剥离 `$...$` 防模仿。`@` 继续 `<qqbot-at-user>`。 |
...
| `chatbot.ts` | 系统短句上色已完成（见「已完成」表）。模型输出永不进公式，保持现状。 |
