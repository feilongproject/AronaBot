    ...
    for (const keyFather in commandFathers)
        for (const keyChild in commandFathers[keyFather]) {
            ...
            const channelAllow: () => boolean = () => {
                for (const allowChannelKey of allowChannels)
                    for (const channel of channelAllows[allowChannelKey])
                        if (channel.id == msg.channel_id) return true;
                return false;
            };
            if (
                devEnv ||
                msg.guild_id == '5237615478283154023' ||
                msg.messageType == MessageType.DIRECT ||
                allowChannels[0] == 'all' ||
                channelAllow()
            ) {
                return { path: keyFather, keyChild, ...opt };
...
class IMessageChannelCommon implements IntentMessage.MessageChannelCommon {
    ...
    async sendMsgEx(_options: Partial<SendOption.Channel> | string): Promise<RetryResult<any>> {
        ...
        options.msgId =
            options.msgId || this.id || (await redis.get(`lastestMsgId:${botType}`)) || undefined;
    ...
    private _sendMsgEx = async (options: Partial<SendOption.Channel>) => {
        ...
        if (options.sendType == MessageType.GUILD)
            return global.client.messageApi
                .postMessage(options.channelId || '', {
                    msg_id: options.msgId,
                    content: content,
                    message_reference:
                        ref && options.msgId ? { message_id: options.msgId } : undefined,
                    image: imageUrl,
                    ark: ark,
                })
                .then((res) => res.data);
        else
            return global.client.directMessageApi
                .postDirectMessage(options.guildId!, {
                    msg_id: options.msgId,
                    content: content,
                    image: imageUrl,
                })
    ...
    async sendMarkdown(options: Partial<SendOption.Channel> & SendOption.MarkdownPublic) {
        options.channelId = options.channelId || this.channel_id;
        options.guildId =
            options.guildId || this.guild_id || options.channelId
                ? Object.entries(saveGuildsTree).find(
                      ([_, cList]) => cList.channels[options.channelId!],
                  )?.[0]
                : undefined;
        options.eventId =
    ...
    async sendToAdmin(content: string) {
        return this.sendMsgEx({
            ...
            guildId: (await redis.hGet(`directUid->Gid:${meId}`, adminId[0])) ?? undefined,
        });
    }

    private sendImage = async (options: SendOption.Channel): Promise<IMessage> => {
        const { sendType, content, imagePath, imageFile, imageUrl, msgId, guildId, channelId } =
            options;
        const pushUrl =
            sendType == MessageType.DIRECT
...
export class IMessageGUILD
    extends IMessageChannelCommon
    implements IntentMessage.GUILD_MESSAGES__body
{
    mentions?: IUser[];
    guildName: string;
    channelName: string;
...
export class IMessageDIRECT
    extends IMessageChannelCommon
    implements IntentMessage.DIRECT_MESSAGE__body
{
    direct_message: true;
    src_guild_id: string;
...
class IMessageChatCommon implements IntentMessage.MessageChatCommon {
    ...
    async sendMsgEx(_options: Partial<SendOption.Chat> | string): Promise<RetryResult<any>> {
        ...
        const uT =
            new URL(options.imageUrl || options.fileUrl || 'http://0').pathname.split('.').pop() ||
            '';
        const fileMap = [[], ['png', 'jpg', 'jpeg'], ['mp4'], ['slik']];
        options.fileType =
            (fileMap.findIndex((v) => v.find((i) => uT.startsWith(i))) as any) || undefined;
    ...
    private _sendFile = async (
        ...
        const fileRes = await (this.messageType == MessageType.GROUP
            ? client.groupApi
                  .postFile(sendToId, {
                      file_type: fileType,
                      file_data: fileData,
    ...
    private _sendMarkdown = async (
        options: Partial<SendOption.Chat> & SendOption.MarkdownOrgin,
    ) => {
...
async function getMarkdown(
    ...
    const kbId =
        options.keyboardId ||
        options.keyboard?.id ||
        (await redis.hGet(`config:kb:${options.keyboardNameId}`, meId));
    const kbContent =
        (options.keyboardNameId
            ? (await import('../../data/keyboardMap')).default[options.keyboardNameId]
            : undefined) || options.keyboard?.content;
