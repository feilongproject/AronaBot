...
export async function alias(msg: IMessageGUILD | IMessageDIRECT | IMessageGROUP): Promise<any> {
    ...
    if (allowMarkdown !== true)
        return msg.sendMsgEx({ content: `ERROR: allowMarkdown is ${allowMarkdown}` });
    const aliasMatch = /(?<un>un)?alias\s+(?<unkName>\S+)(\s+(?<kName>\S+))?/.exec(
        msg.content,
    )?.groups;
    ...
    const mdCmdLink = (showDesc: string, command: string, enter = true) => {
        ...
        return [
            `[${showDesc}]`,
            `(mqqapi://aio/inlinecmd?command=${encodeURIComponent(command)}&reply=false&enter=${enter})`,
            '\r',
        ];
    };
    ...
    if (unkName && kName) {
        ...
        localMap[studentInfo.descName] = localMap[studentInfo.descName].filter(
            (v, i, arr) => arr.indexOf(v, 0) === i,
        ); // 去重
        ...
        return msg
            .sendMsgEx({
                content:
                    `「${unkName}」==>「${studentInfo.descName}」ok` +
    ...
    const btns = searchFuzzy.map((fuzzy) =>
        genBtn(`${fuzzy.name} ${fuzzy.score}`, `${unkownName} ${fixName(fuzzy.name)}`),
    );
    const rows = [...Array(Math.ceil(btns.length / 3))].map((_, i) => ({
        buttons: btns.slice(i * 3, i * 3 + 3),
    }));
...
export async function reloadStudentInfo(
    type: 'net' | 'local',
): Promise<'net ok' | 'local ok' | 'ok'> {
        ...
        for (const _ of nStudentsElectricgoat) {
            ...
            _studentInfo[d.Id] = {
                ...
                name: [
                    ...
                    fixName(d.DevName),
                    String(d.Id),
                    d.PathName ? fixName(d.PathName) : undefined,
            ...
            _studentInfo[d.Id].name = _studentInfo[d.Id].name.filter(
                (v, i, arr) => arr.indexOf(v, 0) === i,
            ); // 去重
    ...
    if (fs.existsSync(config.studentInfo)) {
        ...
        for (const _id in global.studentInfo) {
            ...
            names = names.map((v) => fixName(v)).filter((v, i, arr) => arr.indexOf(v, 0) === i); // 去重
...
export function findStudentInfo(name: string): StudentData | null {
    for (const id in studentInfo)
        if (isNumStr(id) && studentInfo[id].name.includes(fixName(name))) return studentInfo[id];
    return null;
}
...
export function sutdentNameFuzzySearch(source: string, limit = 6): SearchResultScore[] {
    ...
    const searchResult: SearchResultScore[] = uniqBy(
        searchPinyin
            .map((item) => {
                // 拼音一致加1分
                let score = sourceCut
    ...
    return searchResult
        .slice(0, Math.max(maxScoreLength, limit))
        .map((v) => ({ ...v, name: studentInfo[v.id as `${number}`].name[0] }));
}
