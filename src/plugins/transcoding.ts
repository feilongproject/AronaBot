...
export async function loadFile(msg: IMessageC2C) {
    ...
    if (!['mkv', 'ass', 'mp4', 'webm'].includes(ext))
        return msg.sendMsgEx({ content: `不支持的文件格式` });
...
export async function downloadJob(msg: IMessageGROUP | IMessageC2C) {
    ...
    if (nowJob.status !== 200)
        return msg.sendMsgEx({
            content: `文件未准备完毕` + `\nstatus: ${nowJob.status}` + `\nbody: ${nowJob.body}`,
        });
    await msg.sendMsgEx({
        content:
            `开始下载到本地 ${localVideoName}` +
            `\ntmpdir: ${runningJob.tmpdir}` +
            `\nstatus: ${nowJob.status}`,
    });
    ...
    const fileInfo = await msg
        ...
        .catch((err) =>
            msg.sendMsgEx({ content: `获取fileinfo时出现了一些啸问题\n` + strFormat(err) }),
        )
