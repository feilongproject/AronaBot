...
export async function baServerStatus(
    ...
    return Promise.all([jpStatus, globalStatus, cnStatus])
        .then(([jpStatus, globalStatus, cnStatus]) => {
            ...
            const jpContent =
                jpEndTime.getTime() + 1000 * 60 * 60 <= new Date().getTime()
                    ? '日服一切正常, 暂无官方维护通知'
                    : `日服状态:` +
            ...
            const globalStartTime = new Date(
                Number(globalStatus.result.maintenanceInfo?.startTime || 0),
            );
            const globalEndTime = new Date(
                Number(globalStatus.result.maintenanceInfo?.finishTime || 0),
            );
            ...
            const cnContent =
                cnNode && cnEndTime.getTime() > new Date().getTime()
                    ? `国服状态: ${cnNode.title}` +
