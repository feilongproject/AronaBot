...
export async function mute(msg: IMessageGUILD) {
    ...
    const timeMatch = /(?<muteTime>\d+)((?<m>分钟?|m)|(?<h>小?时|h)|(?<d>天|d))/.exec(
        msg.content,
    )?.groups;
    if (!timeMatch) return msg.sendMsgExRef({ content: `未指定禁言时间` });

    const muteTime =
        Number(timeMatch.muteTime) * (timeMatch.m ? 60 : timeMatch.h ? 60 * 60 : 60 * 60 * 24);
    const cmdMatch = execMap.find((v) => v.reg.exec(msg.content)) || {
        t: 'common',
        reg: / /,
        del: false,
        send: false,
    ...
    if (muteTime && cmdMatch.del && !msg.message_reference)
        return msg.sendMsgExRef({ content: `未找到需要撤回消息` });
    ...
    const alart = await client.guildApi
        .guildMember(msg.guild_id, muteMember.id)
        .then((res) => {
            const { data } = res;
    ...
    if (muteTime) {
        ...
        await msg.sendMsgExRef({
            content:
                `已对成员<@${muteMember!.id}>的「${(await redis.hGet('muteType', cmdMatch.t)) || cmdMatch.t}」行为采取禁言${timeConver(muteTime * 1000)}` +
                `\n注意：若管理随意使用则会采取一定措施`,
        });
    } else {
        // TODO: 加点什么
        await msg.sendMsgExRef({
            content: `已解除${(await redis.hGet('muteType', cmdMatch.t)) || cmdMatch.t}禁言`,
        });
    }
    ...
    const sendStr = [
        '禁言记录',
        ...(await Promise.all(
            Object.keys(muteMap).map(
                async (k) =>
                    `时间: ${format.asString(new Date(Number(k)))} | 类型: ${(await redis.hGet('muteType', muteMap[k])) || muteMap[k]}`,
            ),
        )),
    ];
    ...
    if (cmdMatch.send && muteTime) {
        ...
        await msg.sendMsgEx({
            content:
                `<@${muteMember!.id}>(id: ${muteMember!.id})` +
...
export async function ban(msg: IMessageGUILD | IMessageDIRECT | IMessageGROUP | IMessageC2C) {
    ...
    const match =
        /(?<isUnBan>un)?ban(?<isForce>f)?\s+(?<banType>group|guild|user)\s+(?<banId>\S+)(\s+(?<banDesc>\S+))?$/.exec(
            msg.content,
        );
    ...
    if (!isUnBan && isExist && !isForce)
        return msg.sendMsgEx({ content: `已存在该封禁, 理由:\n${isExist}` });
    const result = isUnBan
        ? await redis.hDel(`ban:use:${banType}`, banId)
        : await redis.hSet(`ban:use:${banType}`, banId, banDesc);

    return msg.sendMsgEx({
        content:
            `已${isUnBan || ''}ban ${result} ${isForce && isExist ? '(force)' : ''}` +
            `\n类型: ${banType}` +
            `\nid: ${banId}` +
            (isUnBan ? '' : `\n理由: ${banDesc}`),
    });
