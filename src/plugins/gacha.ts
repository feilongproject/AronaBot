...
async function gachaReload(type: "net" | "local") {
    if (type == "net") return import("./studentInfo").then(module => module.reloadStudentInfo(type)).then(async r => {
        ...
        const schaleDBConfig: SchaleDB.Root = await fetch("https://ghproxy.net/https://raw.githubusercontent.com/lonqie/SchaleDB/main/data/config.min.json").then(res => res.json()).catch(err => log.error(err));
