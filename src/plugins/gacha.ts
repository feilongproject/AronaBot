...
export async function gachaString(msg: IMessageGUILD | IMessageGROUP | IMessageC2C) {
    if (
        ...
        ['5138A9AD154BB8E8E6082B5804E6EB35', 'FCF53BF26F7CA7908601D0C471C22ADC'].includes(
            msg.group_id,
        )
    )
...
export async function gachaImage(msg: IMessageGUILD | IMessageGROUP | IMessageC2C) {
    ...
    const o = cTime(
        setting.server == 'jp' ? 'jp' : 'global',
        10,
        adminId.includes(msg.author.id)
            ? (Number(msg.content.match(/\d$/)) as 1 | 2 | 3)
            : undefined,
    );
    const analyze =
        setting.analyzeHide == 'true'
            ? null
            : await analyzeRandData(setting.server == 'jp' ? 'jp' : 'global', msg.author.id, o);
    ...
    if (devEnv)
        log.debug('start time:', startTime, 'end time:', endTime, 'total:', endTime - startTime);
...
function cTime(server: 'global' | 'jp', times: 1 | 10, testStar?: 1 | 2 | 3): GachaPools {
    ...
    function startRand(type: 'pickup' | 'common', star: 1 | 2 | 3): GachaPool {
        ...
        const _pools =
            type == 'common'
                ? gachaPoolInfo[server][type][star]
                : gachaPoolInfo[server][type].characters;
    ...
    for (var i = 1; i <= times; i++) {
        ...
        if (rNum <= 0.05)
            ret.push({
                ...
                devName: 'Arona',
                star: 3,
                custom: 'NPC_Portrait_Arona.png',
            });
        //彩蛋
        else if (rNum <= 0.7 && gachaPoolInfo[server].pickup.characters.length > 0)
            ret.push(startRand('pickup', 3));
...
async function buildImage(characterNames: GachaPools): Promise<Buffer> {
    ...
    for (const [i, value] of characterNames.entries()) {
        ...
        if (value.custom)
            ...
            files.push({
                input: `${config.images.characters}/Student_Portrait_${value.devName}.png`,
                top: y,
                left: x,
            }); //character avatar
...
export async function reloadGachaData(
...
async function gachaReload(type: 'net' | 'local') {
    if (type == 'net')
        return import('./studentInfo')
            .then((module) => module.reloadStudentInfo(type))
            .then(async (r) => {
                const _gachaPoolInfo: GachaPoolInfo = {
                    global: {
                        common: { 1: [], 2: [], 3: [] },
                        pickup: { characters: [], start: 0, end: 0 },
                    },
                    jp: {
                        common: { 1: [], 2: [], 3: [] },
                        pickup: { characters: [], start: 0, end: 0 },
                ...
                for (const id in studentInfo) {
                    ...
                    for (key in nameToId)
                        if (d.releaseStatus[nameToId[key]] && !d.limitedType[nameToId[key]])
                    ...
                    for (const _nowPick of regions.CurrentGacha)
                        if (_nowPick.start < nowTime && nowTime < _nowPick.end) {
                            for (const pick of _nowPick.characters)
                                if (studentInfo[pick].star == 3)
                                    _gachaPoolInfo[key].pickup.characters.push(pick);
