...
export async function soutubot(msg: IMessageGROUP | IMessageC2C) {
    ...
    const commands = msg.content
        .split(' ')
        .map((v) => v.split('='))
        .filter((v) => v.length > 0 && v.length <= 2) as [string, string?][];
    ...
    const filterData = await Promise.all(
        resData
            ...
            .reduce(
                ...
                { urlMap: new Map(), filteredData: [] as SoutuBot.Data[] },
            )
    ...
    const keyBoardRows = filterData.map((v, i) => {
        if (!SiteMap[v.source])
            return { buttons: [genKeyboard(`${v.source} ${v.pagePath}`, `${v.pagePath}`)] };
        return {
            buttons: [
                ...SiteMap[v.source].variants
                    .map((d) => [
                        genKeyboard(
                            `${d.name}`,
                            `${d.host}${v.subjectPath}`,
                            `${i}-${d.name}-${r()}`,
                        ),
                        genKeyboard(
                            `${d.name}-P${v.page}`,
...
async function notCanUse(msg: IMessageC2C | IMessageGROUP): Promise<boolean> {
    ...
    if (msg instanceof IMessageGROUP && ['963547B9CDEF3C2F51F2C02AC06B808B'].includes(msg.group_id))
        return false;
    if (await redis.sIsMember(`auth:usebot:soutu:user`, msg.author.id)) return false;
    if (
        msg instanceof IMessageGROUP &&
        (await redis.sIsMember(`auth:usebot:soutu:group`, msg.group_id))
    )
        return false;
    return true;
}
