...
export async function wifuToday(msg: IMessageGROUP) {
    ...
    if (!pairUser) {
        if (usedUser.length <= 1)
            return msg.sendMsgEx(`当前群内潜在老婆不足, 请召唤其他群友使用bot任意功能后重新再试`);

        const todayUse: string[] = Object.keys(todayMembers); // 找出今日结婚的
        const notPair = usedUser
            .map((v) => v.aid)
            .filter((v) => !todayUse.includes(v) && v != msg.author.id); // 看看有没有单身的

        if (!notPair.length)
            return msg.sendMsgEx(`今天的老婆被别人全部抢走了...召唤群友使用bot以加入老婆库！`);
