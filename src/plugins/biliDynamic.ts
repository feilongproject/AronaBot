...
export async function mainCheck(
    ...
    for (const item of dynamicItems.items) {
        ...
        const idPushed = await redis.hmGet(
            `biliMessage:idPushed:${dynamicId}`,
            bUser.list.map((v) => v.id),
        );
        ...
        try {
            ...
            const imageBuffer = await screenshot(
                dynamicId,
                item.modules.module_author.pub_ts.toString(),
                30,
            );
            debugger;
            ...
            for (const pushInfo of notPushedList) {
                // 检查未推送的
            ...
            await mailerError(
                { bUser, dynamicId, imageKey, notPushedList, idPushed, item },
                err instanceof Error ? err : new Error(strFormat(err)),
...
async function dynamicPush(
    dynamicId: string,
    pushInfo: DynamicPushList.PushInfo,
    item: BiliDynamic.Item,
    imageKey: string,
    imageBuffer: Buffer,
) {
...
async function screenshot(
    ...
    await page.evaluate(() => {
        ...
        const r = '#app > div > div';
        document
            .querySelector(`#app > div > m-open-app.m-open-app.fixed-openapp.dynamic-float-btn`)
            ?.remove(); // 删除打开 app
        document.querySelector(
            `#app > div > m-open-app.m-open-app.card-wrap > div > div.dyn-header > div.dyn-header__right > div`,
        ); // 删除关注按钮
        ...
        document
            .querySelector(
                `${r}.launch-app-btn.card-wrap > div > div.dyn-header > div.dyn-header__right`,
            )
            ?.remove(); //删除"关注"按钮 (dynamic)
        document
            .querySelector(
                `${r}.launch-app-btn.card-wrap > div > div.dyn-content > div.dyn-content__orig.reference > div.dyn-content__orig__additional`,
            )
            ?.remove(); //删除"相关游戏" (dynamic)

        document.querySelector(`${r}.launch-app-btn.float-openapp.opus-float-btn`)?.remove(); //删除"打开app"按钮
        document
            .querySelector(`${r}.opus-modules > div.opus-module-content.limit`)
            ?.classList.remove('limit'); //"展开阅读全文" (opus)
        document
            .querySelector(`${r}.opus-modules > div.opus-module-content > div.opus-read-more`)
            ?.remove(); //删除"展开阅读全文"阴影 (opus)
        document
            .querySelector(`${r}.opus-modules > div.opus-module-content > div.link-card-para`)
            ?.remove(); //删除"相关游戏" (opus)
        document
            .querySelector(
                `${r}.opus-modules > div.opus-module-author > div.launch-app-btn.opus-module-author__action`,
            )
            ?.remove(); //删除"关注"按钮 (opus)
